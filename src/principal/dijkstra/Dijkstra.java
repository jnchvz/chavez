...
public class Dijkstra {
	private int anchoMapaEnTiles; //width map tiles
	private int altoMapaEnTiles; // height map tiles
	...
	public Point obtenerCoordenadasNodoCoincidente(final Point puntoJugador) { // get coordinates node
		...
		Point puntoExacto = null; // exact point
	...
	private void evaluarHeuristicaVecinos(final Nodo nodo) {
		...
		for (int y = inicialY - 1; y < inicialY + 2; y++) {
			for (int x = inicialX - 1; x < inicialX + 2; x++) {
				//dentro del rango del mapa (-1 en anchoMapaEnTiles??? )  inside the scope of the map
	...
	public Nodo encontrarSiguienteNodoParaEnemigo(Enemigo enemigo) {
		...
		return siguienteNodo; // next node
	}
