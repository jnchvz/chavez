...
public class Mapa {
    ...
    private String[] partes; // parts

    private final int ancho; // width
    private final int alto; // height

    private final Point posicionInicial; // initial position
    private final Point puntoSalida; //exit point

    private Rectangle zonaSalida; // exit zone

    private String siguienteMapa; // next map

    private final Sprite[] paleta; // palette

    private final boolean[] colisiones; // collisions
    ...
    private boolean[] extraerColisiones(final String cadenaColisiones) { // extract collisions
    ...
    private int[] extraerSprites(final String[] cadenasSprites) { // extract sprites
    ...
    private void actualizarZonaSalida() { // update exit zone
    ...
    private void actualizarRecogidaObjetos() { // update picking objects
        if (!objetosMapa.isEmpty()) {
            ...
            for (int i = 0; i < objetosMapa.size(); i++) {
                ...
                final Rectangle posicionContenedor = new Rectangle( // container positionn
