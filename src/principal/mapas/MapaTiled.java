...
public class MapaTiled { // tiled map
	
	private int anchoMapaEnTiles; // width map in tiles
	private int altoMapaEnTiles; // height map in tiles
	
	private Point puntoInicial; // initial point
	
	private ArrayList<CapaSprites> capasSprites; // sprites layers
	private ArrayList<CapaColisiones> capasColisiones; // collision layers
	
	private ArrayList<Rectangle> areasColisionOriginales; // original collision areas
	public ArrayList<Rectangle> areasColisionPorActualizacion; // collision areas by update
	...
	public MapaTiled(final String ruta) {
		String contenido = CargadorRecursos.leerArchivoTexto(ruta); // content
		...
		this.capasSprites = new ArrayList<>(); // sprites layers
		this.capasColisiones = new ArrayList<>(); // collisions layers
		...
		//COMBINE COLLISIONS IN ONE ARRAY - Would make it more efficient 
		...
		//TOTAL EXISTANT SPRITES IN THE LAYERS
		...
		//ASIGN NECESSARY SPRITES TO THE PALETTE FROM LAYERS
		for (int i = 0; i < coleccionesSprites.size(); i++) {
			JSONObject datosGrupo = obtenerObjetoJSON(coleccionesSprites.get(i).toString()); // we convert the object into a string
			
			String nombreImagen = datosGrupo.get("image").toString();
			int anchoTiles = obtenerIntDesdeJSON(datosGrupo, "tilewidth"); // we read the width
			int altoTiles = obtenerIntDesdeJSON(datosGrupo, "tileheight"); // we read the height
			HojaSprites hoja = new HojaSprites("/imagenes/hojasTexturas/" + nombreImagen,
					anchoTiles, altoTiles, false);
			
			// testing if it's working System.out.println("ancho :" + anchoMapaEnTiles + ", alto :" +altoMapaEnTiles);
		...
		for (int i = 0; i < coleccionObjetos.size(); i++) {
			...
			int idObjeto = obtenerIntDesdeJSON(datosObjeto, "id"); // id object
			int cantidadObjeto = obtenerIntDesdeJSON(datosObjeto, "cantidad");  //quantity
			int xObjeto = obtenerIntDesdeJSON(datosObjeto, "x"); // x object
			int yObjeto = obtenerIntDesdeJSON(datosObjeto, "y"); // y object
	...
	public void actualizar() { //update
		actualizarAreasColision(); //update collision areas
		actualizarRecogidaObjetos(); //update picked objects
		
		actualizarEnemigos(); // update enemies
		actualizarAtaques(); // update attacks
	...
	private void actualizarAreasColision() { //update collision areas
	...
	private void actualizarRecogidaObjetos() { //update picking objects
	...
	private void actualizarEnemigos() { //update enemies
		if (!enemigosMapa.isEmpty()) {
	...
	private void actualizarAtaques() { // update attacks

        if (ElementosPrincipales.jugador.obtenerAlcanceActual().isEmpty()
	...
	private JSONObject obtenerObjetoJSON(final String codigoJSON) { // get JSON object
	...
	private JSONArray obtenerArrayJSON(final String codigoJSON) { //get JSON array
		JSONParser lector = new JSONParser();
	...
	public Point obtenerPosicionInicial() { //get initial position
		return puntoInicial;
	}
	
	public Rectangle obtenerBordes(final int posicionX, final int posicionY) { //getting rectangle borders
        ...
        return new Rectangle(x, y, ancho, alto); // x,y,width,height
	}
}
